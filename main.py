...
def create_playground(input_string):
    playground = {'1 1': input_string[0],
                  '1 2': input_string[1],
                  '1 3': input_string[2],
                  '2 1': input_string[3],
                  '2 2': input_string[4],
...
def insert_values(field, input_str):
    for key, value in field.items():
        if key == input_str and value == '_':
            field[key] = 'X'
            return field
        if key == input_str and value != '_':
            return None
...
structure_for_input = create_playground(user_input)

print_grid(field_of_glory)
...
while True:
    ...
    if check_for_none is not None:
        new_structure = check_for_none
        string_for_second_print = convert_field_to_string(new_structure)
        field_for_second_print = build_structures(string_for_second_print, True)
        print_grid(field_for_second_print)
        break
